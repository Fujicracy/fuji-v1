...
describe("Alpha", () => {
  ...
  beforeEach(async() => {
    ...
    await vaultdai.setActiveProvider(creamv1.address);
    await vaultusdc.setActiveProvider(creamv1.address);
    await vaultusdt.setActiveProvider(creamv1.address);

  });

  describe("Alpha CreamFinanceV1 Basic Functionality", () => {

    it("1.- Users[1]: deposit 11.9999 ETH to Vaultdai, check Vaultdai crETH balance Ok", async () => {
      ...
      let vaultbal = await creth.balanceOf(vaultdai.address);
      vaultbal = vaultbal/1;
      let rate = await creth.exchangeRateStored();
      let crethAmount = depositAmount*1e18/rate;
      await expect(vaultbal).to.be.closeTo(crethAmount, 100);

    });

    it("2.- Users[1]: deposit .00001 ETH to a Vault, check theVault crETH balance Ok", async () => {
      ...
      await expect(await thevault.connect(user_X)
            .deposit(depositAmount,
            { value: depositAmount })).to
            .changeEtherBalance(user_X, negdepositAmount);
      let vaultbal = await creth.balanceOf(thevault.address);
      vaultbal = vaultbal/1;
      let rate = await creth.exchangeRateStored();
      let crethAmount = depositAmount*1e18/rate;
      await expect(vaultbal).to.be.closeTo(crethAmount, 100);
    ...
    it("5.- Users[2,4]: Two deposits to Vaultusdc, checks Vaultusdc crETH balance Ok", async () => {
      ...
      let vaultbal = await creth.balanceOf(vaultusdc.address);
      vaultbal = vaultbal/1;
      let rate = await creth.exchangeRateStored();
      let crethAmount = (depositAmount.add(depositAmount_Y))*(1e18/rate);
      await expect(vaultbal).to.be.closeTo(crethAmount, 100);

    });

    it("6.- Users[5]: deposit 10 ETH in Vaultdai and then withdraws 9.9999 ETH, check VaultDai creth balance ok", async () => {
      ...
      await vaultdai.depositBalance(creamv1.address);
    ...
    it("10.- Users[2]: deposits 0.25 ETH collateral to vaultusdt, and malicious Users[0] tries to withdraw 0.125 ETH , reverts", async () => {
      ...
      await expect(await vaultusdt.connect(user_X)
            .deposit(depositAmount,{ value: depositAmount})).to
            .changeEtherBalance(user_X, negdepositAmount);
    ...
    it("11.- Users[16]: deposits 5 ETH to vaultdai, borrows 8000 dai, then paybacks full, and  withdraws all collateral", async () => {
      ...
      await expect(await dai.balanceOf(user_X.address)).to.equal(0);
    ...
    it("13.- Users[11]: Try Deposit-and-Borrow, 3 ETH deposit, 4500 USDT borrow; Vaultusdt Check Balances ", async () => {
      ...
      let depositAmount = ethers.utils.parseEther("3");
      let borrowAmount = ethers.utils.parseUnits("4500",6);

      await expect(await vaultusdt.connect(theCurrentUser)
      ...
      await expect(await usdt.balanceOf(theCurrentUser.address))
            .to.equal(borrowAmount);
