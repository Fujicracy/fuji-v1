...
interface IVaultHarvester {
  function getAaveClaimAddrs() external view returns (address[] calldata);
...
contract VaultETHDAI is IVault, VaultBase, ReentrancyGuard {
  ...
  function harvestRewards(uint256 _farmProtocolNum) external onlyOwner {
    IVaultHarvester vharvest = IVaultHarvester(_fujiAdmin.getVaultHarvester());
    address tokenReturned;

    if (_farmProtocolNum == 1) {
      IAaveLiquidityMining _aaVeLM = IAaveLiquidityMining(
        0xd784927Ff2f95ba542BfC824c8a8a98F3495f6b5
      );

      address[] memory aaveClaimAddrs = vharvest.getAaveClaimAddrs();
      ...
      tokenReturned = 0x4da27a545c0c5B758a6BA100e3a049001de870f5;
    } else {
      tokenReturned = vharvest.collectRewards(_farmProtocolNum);
    }
