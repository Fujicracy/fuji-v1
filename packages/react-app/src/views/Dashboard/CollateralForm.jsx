...
function CollateralForm({ borrowAsset, contracts, provider, address }) {
  ...
  const ethBalanceNumbeer = ethBalance ? Number(formatEther(ethBalance)).toFixed(6) : null
  ...
  const withdraw = async () => {
    const amountNum = Number(amount) === Number(leftCollateral) ? '-1' : amount
    const res = await tx(
      contracts[getVaultName(borrowAsset)].withdraw(parseEther(amountNum), { gasPrice }),
    )
  ...
  const getCollateral = () => {
    ...
    if (action === Action.Withdraw) {
      return collateralBalance.sub(parseEther(amount))
    }

    return collateralBalance.add(parseEther(amount))
  }
  ...
  return (
    <Grid container direction="column">
      ...
      <Grid item>
          ...
          <span className="complementary-infos">
            {action === Action.Supply ? (
              <>
                ...
                <span>
                  {ethBalanceNumbeer ? Number(ethBalanceNumbeer).toFixed(3) : '...'} ETH Ξ
                </span>
              </>
            ) : (
              <>
        ...
        <div className="fake-input">
          <TextField
            ...
            inputRef={register({
              ...
              max: {
                value: action === Action.Supply ? ethBalanceNumbeer : leftCollateral,
                ...
                <InputAdornment position="end">
                  {focus && (
                    <Button
                      ...
                      onClick={() => {
                        setAmount(action === Action.Supply ? ethBalanceNumbeer : leftCollateral)
                        setValue(
                          'amount',
                          action === Action.Supply ? ethBalanceNumbeer : leftCollateral,
                          {
                            shouldValidate: true,
                            shouldDirty: true,
                          },
        ...
        {get(errors, 'amount.message') === 'insufficient-balance' && action === Action.Supply && (
          <Typography className="error-input-msg" variant="body2">
            Insufficient ETH balance
          </Typography>
        )}
        {get(errors, 'amount.message') === 'insufficient-balance' && action === Action.Withdraw && (
          <Typography className="error-input-msg" variant="body2">
