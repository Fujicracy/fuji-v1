...
function InitBorrow({ contracts, provider, address }) {
  ...
  const calcSavedAmount = amount => {
    if (!amount || amount === 0 || !providerAave) return '...'
  ...
  const ethBalanceFormatted = ethBalance ? Number(formatEther(ethBalance)).toFixed(6) : null
  ...
  const neededCollateralFormatted = neededCollateral ? Number(formatEther(neededCollateral)) : null
  ...
  const dialogContents = {
    success: {
      title: 'Success',
      content:
        'Your transaction have been processed, you can now check your position and follow the evolution of your debt position.',
      actions: () => {
        return (
          <DialogActions>
  ...
  const getProviderName = providerOption => {
    let providerName = '...'

    switch (providerOption) {
      case providerAave.address:
        providerName = 'Aave'
        break
      case providerCompound.address:
        providerName = 'Compound'
        break
      case providerDYDX.address:
        providerName = 'dYdX'
        break
      default:
        providerName = '...'
          ...
          <form noValidate>
              ...
              <div className="collateral-input">
                <div className="subtitle">
                  ...
                  <span className="complementary-infos">
                    <span>
                      Your balance:{' '}
                      {ethBalanceFormatted ? Number(ethBalanceFormatted).toFixed(3) : '...'} Ξ
                    </span>
                  </span>
                </div>
                <div className="fake-input">
                  <TextField
                    ...
                    variant="outlined"
                    placeholder={`min ${
                      neededCollateralFormatted ? neededCollateralFormatted.toFixed(3) : '...'
                    }`}
                    onChange={({ target }) => {
                    ...
                    inputRef={register({
                      ...
                      min: { value: neededCollateralFormatted, message: 'insufficient-collateral' },
                      max: { value: ethBalanceFormatted, message: 'insufficient-balance' },
                    })}
                    InputProps={{
                      startAdornment: (
                        <InputAdornment position="start">
                ...
                {get(errors, 'collateralAmount.message') === 'required-amount' && (
                  <Typography className="error-input-msg" variant="body2">
                    Please, type the amount you want to provide as collateral
                  </Typography>
                )}
                {get(errors, 'collateralAmount.message') === 'insufficient-collateral' && (
                  <Typography className="error-input-msg" variant="body2">
                    Please, provide at least{' '}
                    <span className="brand-color">{neededCollateralFormatted.toFixed(3)} ETH</span>{' '}
                    as collateral!
                  </Typography>
                )}
                {get(errors, 'collateralAmount.message') === 'insufficient-balance' && (
                  <Typography className="error-input-msg" variant="body2">
            ...
            <div className="helper">
              <Typography variant="body2">
                You are saving{' '}
                <span>
                  {calcSavedAmount(Number(borrowAmount))} {borrowAsset}
                </span>{' '}
                per year by borrowing with Fuji. <br />
