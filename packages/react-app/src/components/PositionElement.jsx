...
function PositionElement({ position, actionType }) {
  ...
  useEffect(() => {
    ...
    setHealthRatio(logslider(ratios.healthFactor))
  }, [price, position])

  const handleManage = history.push(`/dashboard/position?borrowAsset=${borrowAsset}`)

  let actionEl = null
  switch (actionType) {
    case PositionActions.Manage:
      actionEl = (
        <Button className="position-btn" onClick={handleManage}>
          Manage
        </Button>
      )
      break

    case PositionActions.Liquidate:
      actionEl = <Button className="position-btn">Liquidate</Button>
      break

    default:
      actionEl = <span style={{ width: '5rem' }} />
      break
  }
  ...
  return (
    <div className="position-element">
      ...
      <div className="position-actions">{actionEl}</div>
    </div>
  )
}
