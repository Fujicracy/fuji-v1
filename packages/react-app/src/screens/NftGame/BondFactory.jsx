    ...
    >
      ...
      >
        <Flex flexDirection="row" justifyContent="center" alignItems="center">
          <CountButton backgroundColor={bg} color={color} onClick={() => setAmount(amount - 1)}>
            -
          </CountButton>
          <AmountInput
            value={amount}
          ...
          />
          <CountButton color={color} backgroundColor={bg} onClick={() => setAmount(amount + 1)}>
            +
          </CountButton>
        </Flex>
        <BuyButton mt={isMobile ? '12px' : '16px'} onClick={() => alert('not implemented')}>
          {isBuying && (
            <CircularProgress
              style={{
                width: 20,
